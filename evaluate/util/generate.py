...
def generate_fast2(
    ...
    tokens = tok(prompts, return_tensors="pt", padding=True).to("cuda")
    tok.padding_side = 'right'
    # tokens = {k:v.cuda() for k,v in tokens.items()}
    output = model.generate(**tokens, max_length=max_out_len, do_sample=False)
    txt = []
    for x in output:
        for i in range(len(x)):
            if x[i] != tok.pad_token_id:
                break
        txt.append(tok.decode(x[i:]))
    if isinstance(model, PromptedModel):
        prompts = [model.prompt + x for x in prompts]
    
    # Keep only generated text (no prompt)
    txt = [continuation[len(prompt):].lstrip(' ').lstrip('.') for prompt, continuation in zip(prompts, txt)]
    return txt
