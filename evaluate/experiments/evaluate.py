...
def main(
    ...
    if (
        ...
        or not (run_dir := globals.RESULTS_DIR / dir_name / continue_from_run).exists()
    ):
        continue_from_run = None
    if continue_from_run is None:
        alg_dir = globals.RESULTS_DIR / dir_name
        if alg_dir.exists():
            id_list = [
                int(str(x).split("_")[-1])
            ...
            run_id = 0
        run_dir = globals.RESULTS_DIR / dir_name / f"run_{str(run_id).zfill(3)}"
    ...
    ds = ds_class(globals.DATA_DIR, tok=tok, size=dataset_size_limit)
